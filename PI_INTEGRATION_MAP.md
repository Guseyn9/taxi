...
Ветка `integration/platform-interface`, 7 коммитов, 132 файла, +12 073 / −698.

| Коммит | Блок |
|---|---|
| `32d9150` | interaction-contract + map-channel (18 файлов) |
...
| `ed3b60c` | 12 файлов UI, где версия Гусейна совпадала с базой Ивана |
| `9f02fa4` | 16 файлов ручного объединения |
| `d076858` | Driver/Map.tsx + сопутствующее |
...
| тесты | 42/42 набора, 304/304 теста |
| покрытие дельты Ивана | 134/134 файла |
| `src/platform/**` и `docs/**` | байт-в-байт как у Ивана — публичные contracts не тронуты |
...
(`API.cancelDrive`). Это файл Гусейна, которого разработка Ивана не касалась;
...
1. **`src/tools/markerMock.ts` не перенесён.** 356 строк демо-мока дизайн-системы
   ...
   репозитории остаётся эмулятор Гусейна. `docs/MOCK_MODE.md` перенесён как есть.
...
2. **Подписка на события `DriverMapGateway` в `Map.tsx` частичная.** Обрабатываются
   ...
   обновление состояния после перехода принадлежит обработчикам Гусейна
...
3. **`Failed` логируется без модалки.** Карта Гусейна сознательно не блокирует
...
4. **`onVotingWentClick`** в `CardModal.tsx` и `Order/index.tsx` выражен одним
   ...
   У Гусейна `setOrderState(Arrived)` не был tolerant к `NotAppointedPerformer`,
...
5. **`LegacyRouteProvider` оставлен в версии Ивана.** Попытка делегировать его в
   `tools/route.ts` Гусейна (единый источник геометрии маршрута) замыкает цикл
   импортов `API → localization → state → localization` и роняет 3 набора тестов.
   Дедупликация — отдельная задача.

6. **Имена, оставленные за версией Гусейна:** `refuseVotingOrder` (вместо
   `cancelVotingDeparture`), `onVotingRefuseOrder` (вместо `onVotingCancelDeparture`) —
   используются в JSX Гусейна.

## Осталось сделать

- ревью Ивана по п.22 ТЗ;
- устранение его замечаний;
- PR `integration/platform-interface` → `main` со ссылками на исходные commits
  `91a8426`, `c0c8810`, `6671f69`, `c81f5a2`, `1276759`, `62cea1a`;
