...
при завершении Runtime и не трактуется как серверный `FAILED`: Gateway отклоняет
локальный Promise, но не публикует `driver.order.action.failed`.

Оптимистическая метка отмены водителем устанавливается до отправки команды, чтобы
polling не показал ложное сообщение об отмене клиентом. При любом неподтверждённом
результате (`FAILED`, `TIMEOUT`, `STATUS_LOOKUP` или `CANCELLED`) операция
гарантированно удаляет метку; сохранить её можно только после подтверждённой
отмены.
